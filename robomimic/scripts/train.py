...
def infer_shape_meta_from_hdf5(hdf5_path, used_obs_keys=None):
    hdf5_path = os.path.expanduser(hdf5_path)
    with h5py.File(hdf5_path, "r") as f:
        demo_key = next(iter(f["data"]))                 # e.g., "demo_0"
        d = f["data"][demo_key]
        obs_grp = d["obs"]

        # pick the observation keys you want (or all)
        all_obs_keys = list(obs_grp.keys()) if used_obs_keys is None else used_obs_keys

        # per-step shapes: drop time dimension (typically axis 0)
        all_shapes = {}
        for k in all_obs_keys:
            shp = tuple(obs_grp[k].shape[1:])            # remove time dim
            all_shapes[k] = shp

        ac_dim = int(d["actions"].shape[-1])
        use_images = any(len(all_shapes[k]) >= 3 for k in all_obs_keys)  # crude but fine
